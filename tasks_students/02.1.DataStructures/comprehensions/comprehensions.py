...
def get_unique_user_ids_visited_page_after_ts(
    ...
    return set([record["UserID"] for record in records if record["EventTime"] > ts])
...
def get_record_with_none_if_key_not_in_keys(
    ...
    return dict((key, val) if key in keys else (key, None) for key, val in r.items())
...
def get_record_with_key_in_keys(
    ...
    return {key: val for key, val in r.items() if key in keys}
...
def get_keys_if_key_in_keys(
    ...
    return set(key for key, val in r.items() if key in keys)
